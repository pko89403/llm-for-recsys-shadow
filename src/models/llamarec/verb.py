...
def int_to_letter(n):
    ...
    # Create a list to collect characters
    chars = []
    while n >= 0:
        # Calculate the current character and prepend to the list
        chars.append(chr(n % 26 + 65))
        # Update n for the next iteration
        n = n // 26 - 1
    
    # Reverse the list to correct the order and join into a string
...
class Verbalizer(nn.Module):
    ...
    def _match_label_words_to_label_ids(self, label_words): # TODO newly add function after docs written # TODO rename this function
        ...
        if isinstance(label_words, dict):
            ...
            raise ValueError("Verbalizer label words must belist, tuple or dict")
        return label_words
    ...
    def handle_multi_token(self, label_words_logits, mask):
        ...
        if self.multi_token_handler == "first":
            ...
            label_words_logits = label_words_logits.mas(dim=-1).values
...
class ManualVerbalizer(Verbalizer):
    ...
    def aggregate(self, label_words_logits: torch.Tensor) -> torch.Tensor:
        ...
        """
        label_words_logits = (label_words_logits * self.label_words_mask.to(label_words_logits.device)).sum(-1)/self.label_words_mask.sum(-1).to(label_words_logits.device)
        return label_words_logits
    ...
    def project(self,
        ...
        """
        label_words_logits = logits[:, self.label_words_ids] # ĠA->362, ĠB->426
        label_words_logits = self.handle_multi_token(label_words_logits, self.words_ids_mask.to(label_words_logits.device))
        label_words_logits -= 10000*(1-self.label_words_mask.to(label_words_logits.device))
        return label_words_logits
    ...
    def process_logits(self, logits: torch.Tensor, **kwargs):
        ...
        if self.post_log_softmax:
            # normalize
            label_words_probs = self.normalize(label_words_length)
